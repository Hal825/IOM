...
| Rail | `TaskSidebar` + `TaskItem` | 顶部「队列概况」 + 可滚动「任务列表」；琥珀浅底 |
| Stage | `TaskDetail` + `VideoPlayer` + `Pipeline`（新） | 节点成果卡：视频预览三态 + 任务元信息 + 流水线六阶段 |
| Composer | `Composer` | 移到右列**底部**（编辑器式），白底 + 靛蓝主按钮 |
...
- `TaskSummary = { id, status, progress, text, createdAt, result?, failedReason? }`
