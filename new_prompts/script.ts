/**
 * Script 节点 — 新版本 Prompt 模板(解耦四子脚本,用于评测对比).
 *
 * 与旧版(new_prompts/script.ts 历史的 project + scenes[] 一体化结构)不同,
 * 本版本将 script 节点的输出拆成四个职责单一的子脚本:
 *   - storyScript      剧情脚本(发生什么故事)
 *   - storyboardScript 分镜脚本(怎么拍 + 资源引用 + 技术参数)
 *   - audioScript      音频脚本(听什么,供 TTS / 音频合成)
 *   - pacingScript     节奏脚本(时间分配与转场)
 *
 * 本轮仅用于评测对比,不改动生产代码(lib/agent、lib/tools、lib/types.ts).
 * 运行 scripts/eval-script.ts(新 Proposal → 新 Script)对比效果.
 */
...
- **sceneVisuals[]**:每个空间的 visualId,description,visualHints,及其下的 scenes[](sceneId,sceneDescription,duration)
